...
func _import() -> Error:
	...
	for source_file in source_files:
		if not FileAccess.file_exists(source_file):
			...
			status.text += '\nFile not found at path "%s"!' % source_file
			continue;
		...
		while xml.read() == OK:
			...
			if node_name == 'textureatlas':
				...
				if not FileAccess.file_exists(image_path):
					...
					status.text += '\nImage not found at %s.' % [image_name]
		...
		var err = ResourceSaver.save(sprite_frames, filename, ResourceSaver.FLAG_COMPRESS)
		if err == OK:
			status.text += 'Saved to %s\n' % [filename]
		else:
			status.text += 'Error when saving %s\n' % [filename]
...
func _set_files(paths: PackedStringArray) -> void:
	source_files = paths;
	
	file.text = '';
	for path in paths:
		file.text += '%s, ' % [path.get_file()]
